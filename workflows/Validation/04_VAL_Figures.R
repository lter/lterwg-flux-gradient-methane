...
load(file = fs::path(localdir,paste0("/Validation_Sites/Val_PARMS_DIEL_Q10_RSHPc_wide.Rdata")))
...
plot.totaldaily <- ENSEMBLE_DIELSc_wide  %>% 
  ...
  theme_bw() +
  scale_color_discrete_qualitative( palette = "Dark2") + ylab(expression(paste( " EC CH"[4], " ( mg C m"^-2, " day" ^-1, ")")))  +
  xlab(expression(paste( " GF CH"[4], " ( mg C m"^-2, " day" ^-1, ")"))) +
...
plot.daily.boxplot <- ENSEMBLE_DIELSc_wide  %>% ggplot( ) +
  ...
  ylab(expression(paste( " GF CH"[4], " ( mg C m"^-2, " day" ^-1, ")"))) +
  labs(color = "") + xlab("")+ facet_wrap(~site, scales = "free")
