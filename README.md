...
- `12_SourceProp_MagnitudeModels.R` — Fits the two-stage Random Forest model: Stage 1 is a balanced binary source-probability classifier (P(source)); Stage 2 is a log-magnitude regression, fit separately for weak-sink and weak-source conditions. Climatically "Arid" site-months (JORN, SRER) are excluded from Stage 1 and from Stage 2's source-magnitude model, and are instead routed to a forced-sink prediction — see the script's header docstring for the empirical rationale. Stage 2 performance is now reported with Lin's concordance correlation coefficient (CCC) decomposed into precision (r) and accuracy (C_b), plus mean bias, alongside RMSE/MAE. Static site attributes (EcoType, MAP, MAT) are drawn from `OUTPUT/NEON_scale_annual_budget_summary.csv` (regenerated by step 05) rather than the orphaned `OUTPUT/30min_site_behavior.csv`. Saves a model bundle consumed (not refit) by step 13.
...
- `14_Global_SpatialUpscalingFiguresRF.R` — Standalone figure script for the RF upscaling pipeline; loads pre-computed outputs from step 13 (and, for one figure, step 12) and regenerates all publication figures without re-running the projection. Observed-vs-fitted magnitude panels are annotated with CCC (and its accuracy factor C_b) and mean bias.
...
- `20_Supp_NEON_SiteMetadata_MODIS_Inundation_Table.R` — Builds a per-site attribute table for the NEON CH4 flux-gradient sites, combining NEON/AmeriFlux metadata (IGBP class and site description), MODIS MCD12C1 land cover (both the raw IGBP majority class and the project's derived 5-class ecotype), and the range (min/max/mean) of WAD2M monthly inundated fraction (Fw, 2000–2020) at each site's 0.5° cell. Also flags whether each site is included in the upland source/sink models (steps 12/13). Writes `OUTPUT/NEON_site_metadata_landcover_inundation.csv`. Standalone supplemental table (like 12b/12c) — not sourced by the master `WorkFlow.R`; requires the MODIS files from step 11.
...
workflows/
  ...
  20_Supp_*.R           # Standalone supplemental site-metadata table (not in main workflow)
...
Note: figure scripts have been cleaned up so panel titles/captions live in the manuscript rather than baked into the images (panels now carry only letter tags), and main-text figure outputs are named by their manuscript number (e.g., `FIGURE3_*`, `FIGURE4_*`).
